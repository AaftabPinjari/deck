    ...
    return (
        <td className="border border-neutral-300 dark:border-neutral-700 min-w-[100px] relative group/cell p-0 align-top">
            <div
                ...
                className={cn(
                    "w-full h-full min-h-[32px] p-2 outline-none",
                    !readOnly && "focus:bg-blue-50 dark:focus:bg-blue-900/20"
                )}
                onInput={(e) => !readOnly && onChange(e.currentTarget.innerText)}
                onBlur={(e) => !readOnly && onChange(e.currentTarget.innerText)}
            />
            {isFirstRow && onDeleteCol && !readOnly && (
                <button
            ...
            {isLastCol && onDeleteRow && !readOnly && (
                <button
                    contentEditable={false}
...
export function TableBlock({ block, onUpdate, readOnly }: TableBlockProps) {
    ...
    }, [block, onUpdate, tableData, readOnly]);
